...
async function fetchActiveViewByLineItem(lineItemIds, networkCode, token) {
  ...
  const queryXml = `
    ...
    <columns>TOTAL_ACTIVE_VIEW_AVERAGE_VIEWABLE_TIME</columns>
  ...
  // CSV: LINE_ITEM_ID, VIEWABLE, MEASURABLE, AVG_VIEWABLE_TIME(seconds). avg time is an average over
  // the LI's viewable impressions, so aggregate it impression-weighted (Σ time·viewable) for the "attention" dwell signal.
  ...
  for (const line of lines.slice(1)) {
    ...
    const avgTime    = parseFloat(cols[3] || '0'); // seconds
    if (lineItemId && measurable > 0) {
      if (!activeViewByLineItem[lineItemId]) activeViewByLineItem[lineItemId] = { viewable: 0, measurable: 0, viewableTime: 0 };
      activeViewByLineItem[lineItemId].viewable     += viewable;
      activeViewByLineItem[lineItemId].measurable   += measurable;
      activeViewByLineItem[lineItemId].viewableTime += (isNaN(avgTime) ? 0 : avgTime) * viewable;
    }
  }
...
async function fetchActiveViewByCreative(lineItemIds, networkCode, token) {
  ...
  const csvText = await runReportAndDownload(queryXml, networkCode, token);
  const lines = csvText.split('\n').filter(Boolean);
  // CSV: LINE_ITEM_ID, CREATIVE_ID, VIEWABLE, MEASURABLE, IMPRESSIONS, AVG_VIEWABLE_TIME(seconds)
  ...
  for (const line of lines.slice(1)) {
    ...
    const avgTime     = parseFloat(cols[5] || '0'); // seconds; impression-weighted below
    if (lineItemId && creativeId && creativeId !== '0' && (measurable > 0 || impressions > 0)) {
      ...
      if (!avByLIAndCreative[lineItemId][creativeId]) avByLIAndCreative[lineItemId][creativeId] = { viewable: 0, measurable: 0, impressions: 0, viewableTime: 0 };
      ...
      avByLIAndCreative[lineItemId][creativeId].viewableTime += (isNaN(avgTime) ? 0 : avgTime) * viewable;
    }
  }
