 ...
 *
 * Returns { [lineItemId]: { [creativeId]: { [keyName]: { [value]: { impressions, clicks } } } } }.
 * Broken out by CREATIVE_ID (the rendered report creative ID) so scripts/refresh.js can attribute
 * each dashboard row to ITS OWN creatives — a single line item often carries several creatives, and
 * without the creative dimension every row on a shared line item would blend in its siblings.
...
// AUDIENCE keys ("AP Full Reach" demographics) that also come via CUSTOM_CRITERIA but whose
// values are numeric codes needing name resolution (like permutive): gender, age, household
// size, income, employment, education, children-in-household. Kept separate from CONTEXTUAL
// keys so they never win the contextual chip — they show as audience lenses in the modal.
const AUDIENCE_CRITERIA_KEYS = ['ap_gen', 'ap_stda', 'ap_hhs', 'ap_inc', 'ap_emp', 'ap_edu', 'ap_chi'];
...
async function fetchSegmentPerformance(lineItemIds, permutiveKeyId, networkCode, token, opts = {}) {
  // Adding CREATIVE_ID multiplies each line item's rows by its creative count, so use a smaller
  // batch than the line-item-only report did to keep each report/download a safe size.
  const { days = 1094, batchSize = 25 } = opts;
  const out = {}; // liId -> creativeId -> keyName -> value -> { impressions, clicks }
  ...
  const add = (liId, cid, key, value, imps, clicks) => {
    if (!liId || !cid || !value) return;
    const L = out[liId] || (out[liId] = {});
    const C = L[cid] || (L[cid] = {});
    const K = C[key] || (C[key] = {});
    const cur = K[value] || (K[value] = { impressions: 0, clicks: 0 });
  ...
  for (let i = 0; i < lineItemIds.length; i += batchSize) {
    ...
    try {
      const q = `<dimensions>LINE_ITEM_ID</dimensions><dimensions>CREATIVE_ID</dimensions><dimensions>CUSTOM_CRITERIA</dimensions>`
      ...
      // Header: Dimension.LINE_ITEM_ID, Dimension.CREATIVE_ID, Dimension.CUSTOM_CRITERIA,
      //         Dimension.CUSTOM_TARGETING_VALUE_ID, Column.TOTAL_..._IMPRESSIONS, Column.TOTAL_..._CLICKS
      for (let j = 1; j < lines.length; j++) {
        if (!lines[j]) continue;
        const cols = splitCsvLine(lines[j]);
        const liId = cols[0];
        const cid = cols[1];                   // CREATIVE_ID (rendered report creative ID)
        const criteria = cols[2] || '';        // "key=value"
        const valueId = cols[3] || '';         // CUSTOM_TARGETING_VALUE_ID
        ...
        if (CRITERIA_KEYS.includes(key)) {
          ...
          add(liId, cid, key, value, isNaN(imps) ? 0 : imps, isNaN(clicks) ? 0 : clicks);
          ...
          add(liId, cid, key, valueId, isNaN(imps) ? 0 : imps, isNaN(clicks) ? 0 : clicks);
        }
    ...
    if (permutiveKeyId) {
      try {
        const q = `<dimensions>LINE_ITEM_ID</dimensions><dimensions>CREATIVE_ID</dimensions><dimensions>CUSTOM_DIMENSION</dimensions>`
                + `<columns>TOTAL_LINE_ITEM_LEVEL_IMPRESSIONS</columns><columns>TOTAL_LINE_ITEM_LEVEL_CLICKS</columns>`
                + `<customDimensionKeyIds>${permutiveKeyId}</customDimensionKeyIds>`
                + `${dateXml}<statement><query>WHERE LINE_ITEM_ID IN (${inClause})</query></statement>`;
        const csv = await runReportAndDownload(q, networkCode, token);
        const lines = csv.split('\n');
        // Header: Dimension.LINE_ITEM_ID, Dimension.CREATIVE_ID,
        //         Dimension.TOP_LEVEL_CUSTOM_TARGETING_KEY[ID]_VALUE, Dimension...._ID, Column.IMPS, Column.CLICKS
        for (let j = 1; j < lines.length; j++) {
          if (!lines[j]) continue;
          const cols = splitCsvLine(lines[j]);
          const liId = cols[0];
          const cid = cols[1];           // CREATIVE_ID (rendered report creative ID)
          const code = cols[2] || '';    // permutive value code (e.g. "bhgs", "88829")
          const valueId = cols[3] || ''; // GAM custom-targeting value ID (resolves to a name)
          ...
          add(liId, cid, 'permutive', valueId, isNaN(imps) ? 0 : imps, isNaN(clicks) ? 0 : clicks);
        }
      } catch (e) {
  ...
  if (audIds.length) {
    ...
    for (const liId of Object.keys(out)) {
      for (const cid of Object.keys(out[liId])) {
        for (const ak of AUDIENCE_KEYS) {
          const m = out[liId][cid][ak];
          if (!m) continue;
          const relabeled = {};
          for (const [vid, stats] of Object.entries(m)) {
            const resolved = labels[vid] && labels[vid].value;
            // Permutive: drop values GAM has no readable name for — the resolved label falls
            // back to the raw code (e.g. "rts"), which isn't a meaningful audience segment.
            if (ak === 'permutive' && (!resolved || resolved === audienceCodeById[vid])) continue;
            const name = resolved || audienceCodeById[vid] || vid;
            const cur = relabeled[name] || (relabeled[name] = { impressions: 0, clicks: 0 });
            cur.impressions += stats.impressions; cur.clicks += stats.clicks;
          }
          if (Object.keys(relabeled).length) out[liId][cid][ak] = relabeled;
          else delete out[liId][cid][ak];
        }
      }
    }
