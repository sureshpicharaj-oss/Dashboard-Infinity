...
async function fetchSegmentPerformance(lineItemIds, permutiveKeyId, networkCode, token, opts = {}) {
  ...
  const permutiveCode = {}; // permutive GAM value ID -> its short code (fallback label)
  ...
  // Relabel permutive segments from their GAM value ID to the readable name (displayName),
  // e.g. value ID 449090446926 → "Automotive/Auto Shows". Falls back to the short code
  // (e.g. "rts") when a value has no displayName, or if resolution fails entirely.
  const permIds = Object.keys(permutiveCode);
  if (permIds.length) {
    let labels = {};
    try {
      labels = await resolveCustomTargetingValues(permIds, networkCode, token);
    } catch (e) {
      console.warn(`Permutive name resolution failed: ${e.message} — using codes`);
    }
    const labelFor = id => (labels[id] && labels[id].value) || permutiveCode[id] || id;
    for (const liId of Object.keys(out)) {
      const perm = out[liId].permutive;
      if (!perm) continue;
      const relabeled = {};
      for (const [vid, stats] of Object.entries(perm)) {
      ...
      out[liId].permutive = relabeled;
    }
    console.log(`Permutive: resolved ${Object.keys(labels).length}/${permIds.length} segment names`);
  }
