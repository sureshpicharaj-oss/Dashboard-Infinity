...
/**
 * Resolves a list of custom-targeting key NAMES to their numeric IDs.
 * Returns { [name]: id } for the keys that exist. Used by the segment-performance
 * pipeline, which needs the permutive key's ID for a CUSTOM_DIMENSION report.
 */
async function resolveCustomTargetingKeyIds(names, networkCode, token) {
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <getCustomTargetingKeysByStatement xmlns="${GAM_SOAP_NS}">
      <filterStatement><query>WHERE name IN (${nameList}) LIMIT 100</query></filterStatement>
    </getCustomTargetingKeysByStatement>
  </soapenv:Body>
</soapenv:Envelope>`;
  const res = await withRetry(() => axios.post(GAM_CUSTOM_TARGETING_ENDPOINT, soap, {
    headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 15000,
  }), { label: 'resolveCustomTargetingKeyIds' });
  const parsed = await xml2js.parseStringPromise(res.data);
  const keys = parsed['soap:Envelope']?.['soap:Body']?.[0]?.['getCustomTargetingKeysByStatementResponse']?.[0]?.rval?.[0]?.results || [];
  const out = {};
  for (const k of keys) { const n = k.name?.[0]; const id = k.id?.[0]; if (n && id) out[n] = id; }
  return out;
}

module.exports = { resolveVideoTargetingKeyIds, findCustomTargetingValueIds, getCustomTargetingValueMap, resolveCustomTargetingValues, resolveCustomTargetingKeyIds };
