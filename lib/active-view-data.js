...
async function computeActiveView(urlLineItemMap, urlLicaImpsMap, networkCode, token) {
  ...
  if (urlLicaImpsMap) {
    ...
    for (const p of pairs) {
      ...
      if (!totals[p.url]) totals[p.url] = { viewable: 0, measurable: 0, viewableTime: 0 };
      totals[p.url].viewable     += rec.viewable;
      totals[p.url].measurable   += rec.measurable;
      totals[p.url].viewableTime += rec.viewableTime || 0;
      ...
      const pc = perCreativeRaw[p.url][p.creativeId] || (perCreativeRaw[p.url][p.creativeId] = { viewable: 0, measurable: 0, viewableTime: 0 });
      pc.viewable     += rec.viewable;
      pc.measurable   += rec.measurable;
      pc.viewableTime += rec.viewableTime || 0;
      applied++;
    }
    ...
    for (const [url, t] of Object.entries(totals)) {
      if (t.measurable > 0) {
        activeViewByUrl[url] = {
          ...
          avgViewableSec: t.viewable > 0 ? parseFloat((t.viewableTime / t.viewable).toFixed(1)) : null,
        };
      }
    }
  } else {
    ...
    for (const [url, lineItemIds] of Object.entries(urlLineItemMap)) {
      let totalViewable = 0, totalMeasurable = 0, totalViewableTime = 0;
      for (const lid of lineItemIds) {
        const av = activeViewByLineItem[lid];
        if (av) { totalViewable += av.viewable; totalMeasurable += av.measurable; totalViewableTime += av.viewableTime || 0; }
      }
      if (totalMeasurable > 0) {
        activeViewByUrl[url] = {
          ...
          avgViewableSec: totalViewable > 0 ? parseFloat((totalViewableTime / totalViewable).toFixed(1)) : null,
        };
      }
    }
  }
