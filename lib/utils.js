...
/**
 * Retries an async operation — typically a single GAM SOAP/REST axios call — on
 * transient failures (request timeout, dropped connection, DNS blip, or a 5xx/429
 * response) with exponential backoff. Non-transient failures (a 4xx other than 429,
 * or any error carrying an HTTP response body such as a SOAP fault) throw immediately
 * so genuine problems aren't masked or silently delayed.
 *
 * This exists because the unattended daily refresh (scripts/refresh.js) makes ~20
 * heavy SOAP calls in sequence; before this, a single slow GAM response — GAM
 * periodically takes >30s to return a large getLineItemsByStatement page — would
 * throw and discard the entire day's refresh. Wrap ONLY the network call, never the
 * XML parsing, so a parse bug is not retried.
 */
async function withRetry(fn, { retries = 3, baseDelayMs = 2000, label = 'GAM request' } = {}) {
  let lastErr;
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const transient =
        err.code === 'ECONNABORTED' ||   // axios request timeout
        err.code === 'ETIMEDOUT' ||
        err.code === 'ECONNRESET' ||
        err.code === 'ENOTFOUND' ||
        err.code === 'EAI_AGAIN' ||
        (!err.response && !!err.request) ||             // request made, no response (network-level)
        status === 429 || (status >= 500 && status < 600);
      if (!transient || attempt === retries) throw err;
      const delay = baseDelayMs * Math.pow(2, attempt - 1);
      console.warn(`[retry] ${label} failed (${err.code || status || err.message}); attempt ${attempt}/${retries}, retrying in ${delay}ms`);
...
module.exports = { fetchAdvertiserName, slugToName, getTemplateVarValue, extractNetlifyUrl, splitCsvLine, withRetry };
