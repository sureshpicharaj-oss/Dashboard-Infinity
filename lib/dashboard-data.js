...
async function fetchDashboardData(networkCode, token) {
  ...
  for (const { creative, netlifyUrl, videoId, isMobile } of netlifyCreatives) {
    ...
    if (!grouped[groupKey]) grouped[groupKey] = { netlifyUrl: baseUrl, videoId: videoId || null, isMobile, impressions: 0, clicks: 0, lineItemIds: new Set(), adUnitIds: new Set(), creativeIds: new Set(), firstStartDate: Infinity };
  ...
  const results = Object.values(grouped)
    ...
    .map(r => {
      ...
      return {
        ...
        adUnitNames:          [...r.adUnitIds].map(id => AD_UNIT_NAMES[id]).filter(Boolean),
