...
async function fetchVideoCompletionByLineItem(lineItemIds, networkCode, token) {
  ...
  // Each row: LINE_ITEM_ID, CREATIVE_ID, CREATIVE_NAME, VIDEO_VIEWERSHIP_START, VIDEO_VIEWERSHIP_COMPLETE
  const rawByLineItem = {};
  for (const line of lines.slice(1)) {
    const cols = line.split(',').map(c => c.trim().replace(/"/g, ''));
    const liId        = cols[0];
    ...
    const starts      = parseFloat(cols[3]) || 0;
    const completes   = parseFloat(cols[4]) || 0;
    if (!liId) continue;
    if (!rawByLineItem[liId]) rawByLineItem[liId] = { byCreative: {}, totStarts: 0, totCompletes: 0 };
    rawByLineItem[liId].totStarts    += starts;
    rawByLineItem[liId].totCompletes += completes;
    if (creativeId && creativeId !== '0' && starts > 0) {
      if (!rawByLineItem[liId].byCreative[creativeId]) rawByLineItem[liId].byCreative[creativeId] = { starts: 0, completes: 0, name: creativeName };
  ...
  for (const [liId, raw] of Object.entries(rawByLineItem)) {
    ...
    for (const [cId, c] of Object.entries(raw.byCreative)) {
      byCreative[cId] = { rate: parseFloat(((c.completes / c.starts) * 100).toFixed(1)), name: c.name || '', starts: c.starts };
    }
    const aggregate = raw.totStarts > 0 ? parseFloat(((raw.totCompletes / raw.totStarts) * 100).toFixed(1)) : null;
    byLineItem[liId] = { byCreative, aggregate, totStarts: raw.totStarts };
  }
