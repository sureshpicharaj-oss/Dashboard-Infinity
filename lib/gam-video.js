...
const { splitCsvLine, withRetry } = require('./utils');
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    ...
    const valRes = await withRetry(() => axios.post(GAM_CUSTOM_TARGETING_ENDPOINT, valSoap, {
      headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 15000,
    }), { label: 'getCustomTargetingValues(video)' });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    ...
    const parentRes = await withRetry(() => axios.post(GAM_INVENTORY_SOAP_ENDPOINT, parentSoap, {
      headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 15000,
    }), { label: 'getAdUnitsByStatement(parent)' });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <getAdUnitsByStatement xmlns="${GAM_SOAP_NS}">
      ...
      const invRes = await withRetry(() => axios.post(GAM_INVENTORY_SOAP_ENDPOINT, invSoap, {
        headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 15000,
      }), { label: 'getAdUnitsByStatement(flix)' });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <getLineItemsByStatement xmlns="${GAM_SOAP_NS}">
      ...
      const liRes = await withRetry(() => axios.post(GAM_LINEITEM_SOAP_ENDPOINT, liSoap, {
        headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 60000,
      }), { label: `getLineItemsByStatement(${liType} offset ${offset})` });
      ...
      for (const li of lineItems) {
        ...
        // Name-based fallback for line items whose custom targeting does not include the video
        // value. Requires an exact token match, not a raw substring — a short VIDEO_ID like
        // "Sky" must not match an unrelated line item whose name merely contains it as a
        // substring (e.g. "Flix_SkyGlass_Q3").
        const liNameTokens = liName.split(/[^a-zA-Z0-9]+/).filter(Boolean);
        for (const vid of uniqueIds) {
          if (!videoIdToLineItemIds[vid] && liNameTokens.includes(vid)) {
  ...
  <soapenv:Body>
    <getLineItemCreativeAssociationsByStatement xmlns="${GAM_SOAP_NS}">
      <filterStatement><query>WHERE lineItemId IN (${liIdList}) LIMIT ${licaPageSize} OFFSET ${licaOffset}</query></filterStatement>
        ...
        const licaRes = await withRetry(() => axios.post(GAM_LICA_SOAP_ENDPOINT, licaSoap, {
          headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 60000,
        }), { label: `getLICAsByStatement(video offset ${licaOffset})` });
      ...
      for (const lica of allLicas) {
          ...
          for (const [videoName, data] of Object.entries(videoIdToLineItemIds)) {
            if (data.creativeTargetingName === tName && data.lineItemIds.includes(liId)) {
              if (!data.trackingCreativeId) {
                ...
                data.trackingLineItemId = liId; // the LI hosting the tracking creative (for a GAM creative deep-link)
        ...
        for (const valId of licaValueIds) {
          ...
          if (videoName && cId && videoIdToLineItemIds[videoName] && !videoIdToLineItemIds[videoName].trackingCreativeId) {
            ...
            videoIdToLineItemIds[videoName].trackingLineItemId = liId; // LI hosting the tracking creative
            const licaStats = lica.stats?.[0];
            const licaStarts    = parseInt(licaStats?.videoStartsDelivered?.[0]    || '0');
            const licaCompletes = parseInt(licaStats?.videoCompletionsDelivered?.[0] || '0');
            if (licaStarts > 0) {
  ...
  <soapenv:Body>
    <getCreativesByStatement xmlns="${GAM_SOAP_NS}">
      <filterStatement><query>WHERE id IN (${creIdList}) LIMIT 500</query></filterStatement>
        ...
        const creRes = await withRetry(() => axios.post(GAM_SOAP_ENDPOINT, creSoap, {
          headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 30000,
        }), { label: 'getCreativesByStatement(video durations)' });
        ...
        // Duration is resolved via the video's own trackingCreativeId first; the
        // first-creative-in-the-line-item heuristic is only used as a fallback, and only
        // when the line item hosts exactly one VIDEO_ID — for a line item hosting multiple
        // videos there's no way to tell which creative's duration belongs to which video.
        const liVideoCount = {};
        for (const data of Object.values(videoIdToLineItemIds)) {
          for (const liId of data.lineItemIds) liVideoCount[liId] = (liVideoCount[liId] || 0) + 1;
        }
        for (const [vid, data] of Object.entries(videoIdToLineItemIds)) {
          ...
          if (data.trackingCreativeId && creIdToDurationMs[data.trackingCreativeId]) {
            data.durationMs = creIdToDurationMs[data.trackingCreativeId];
            continue;
          }
          for (const liId of data.lineItemIds) {
            if (liVideoCount[liId] !== 1) continue;
  ...
  <soapenv:Body>
    <getCreativesByStatement xmlns="${GAM_SOAP_NS}">
      <filterStatement><query>WHERE name IN (${nameList}) LIMIT 500</query></filterStatement>
            ...
            try {
              const nsRes = await withRetry(() => axios.post(GAM_SOAP_ENDPOINT, nameSearchSoap, {
                headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 30000,
              }), { label: `getCreativesByStatement(name batch ${i})` });
...
async function fetchVideoCompletionByLineItem(lineItemIds, networkCode, token) {
  ...
  // Each row: LINE_ITEM_ID, CREATIVE_ID, CREATIVE_NAME, DEVICE_CATEGORY_NAME, DEVICE_CATEGORY_ID (auto-added by GAM), VIDEO_VIEWERSHIP_START, VIDEO_VIEWERSHIP_COMPLETE
  // CREATIVE_NAME is free text and can contain commas, so this uses a quote-aware split —
  // a naive line.split(',') would shift every column after a comma in the name.
  const rawByLineItem = {};
  for (const line of lines.slice(1)) {
    ...
    const device      = cols[3] || '';
    // cols[4] = DEVICE_CATEGORY_ID (auto-added by GAM alongside DEVICE_CATEGORY_NAME — skip it)
    const starts      = parseFloat(cols[5]) || 0;
    const completes   = parseFloat(cols[6]) || 0;
    if (!liId) continue;
...
async function fetchVideoCompletionByCreativeId(creativeIds, networkCode, token) {
  ...
  for (const [cId, raw] of Object.entries(byCreativeId)) {
    if (raw.starts > 0) result[cId] = { rate: parseFloat(((raw.completes / raw.starts) * 100).toFixed(1)), starts: raw.starts, completes: raw.completes };
  }
