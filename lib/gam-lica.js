...
async function fetchCreativeLICAStats(netlifyIds, networkCode, token) {
  ...
  const clicksByCreativeAndLI = {}; // per (creative, line item) clicks — for the report-reconciliation fallback
  const batchSize = 100;
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <getLineItemCreativeAssociationsByStatement xmlns="${GAM_SOAP_NS}">
      ...
      const res = await withRetry(() => axios.post(GAM_LICA_SOAP_ENDPOINT, soap, {
        headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` },
        timeout: 60000,
      }), { label: `LICA statsByCreativeId(offset ${offset})` });
        ...
        if (creativeId) {
          ...
          if (lineItemId) {
            ...
            if (!isNaN(clicks) && clicks > 0) {
              if (!clicksByCreativeAndLI[creativeId]) clicksByCreativeAndLI[creativeId] = {};
              clicksByCreativeAndLI[creativeId][lineItemId] = (clicksByCreativeAndLI[creativeId][lineItemId] || 0) + clicks;
            }
          }
        }
      }
  ...
  return { statsByCreativeId, lineItemsByCreativeId, impsByCreativeAndLI, clicksByCreativeAndLI };
}
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    ...
    }), { label: `excludedOrders lineItems(offset ${liOffset})` });
  ...
  <soapenv:Body>
    <getLineItemCreativeAssociationsByStatement xmlns="${GAM_SOAP_NS}">
      ...
      const licaRes = await withRetry(() => axios.post(GAM_LICA_SOAP_ENDPOINT, licaSoap, {
        headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 30000,
      }), { label: `excludedOrders LICAs(offset ${licaOffset})` });
