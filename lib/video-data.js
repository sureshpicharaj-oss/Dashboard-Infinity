...
async function fetchVideoStats(videoIdMap, networkCode, token) {
  ...
  for (const vid of allVideoIds) {
    ...
    for (const device of deviceList) {
      ...
      // The creative + line item the completion rate is measured on (the video's tracking creative),
      // so the UI can deep-link the Video ID straight to that GAM creative. This is deliberately the
      // completion-source creative, NOT a split's own display creative — they can differ.
      if (data.trackingCreativeId) {
        entry.complCreativeId = data.trackingCreativeId;
        const complLi = data.trackingLineItemId || (data.lineItemIds || [])[0];
        if (complLi) entry.complLineItemId = complLi;
      }
