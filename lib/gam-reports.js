...
const { extractNetlifyUrl, splitCsvLine, withRetry } = require('./utils');
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  let runRes;
  try {
    runRes = await withRetry(() => axios.post(GAM_REPORT_SOAP_ENDPOINT, runSoap, { headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 30000 }), { label: 'runReportJob' });
  } catch(axErr) {
  ...
  const csvRes = await withRetry(() => axios.get(downloadUrl, { headers: { Authorization: `Bearer ${token}` }, timeout: 60000, responseType: 'arraybuffer' }), { label: `downloadReport(${jobId})` });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  const runRes = await withRetry(() => axios.post(GAM_REPORT_SOAP_ENDPOINT, runSoap, {
    headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` },
    timeout: 30000,
  }), { label: 'runReportJob(impressions)' });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    ...
    const statusRes = await withRetry(() => axios.post(GAM_REPORT_SOAP_ENDPOINT, statusSoap, {
      headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` },
      timeout: 15000,
    }), { label: `getReportJobStatus(${jobId})` });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  const urlRes = await withRetry(() => axios.post(GAM_REPORT_SOAP_ENDPOINT, urlSoap, {
    headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` },
    timeout: 15000,
  }), { label: `getReportDownloadURL(${jobId})` });
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body><getCreativesByStatement xmlns="${GAM_SOAP_NS}"><filterStatement>
    ...
    const res = await withRetry(() => axios.post(GAM_SOAP_ENDPOINT, soap, { headers: { 'Content-Type': 'text/xml; charset=UTF-8', 'SOAPAction': '', 'Authorization': `Bearer ${token}` }, timeout: 30000 }), { label: `lookupNetlifyUrls(batch ${i})` });
